...
object UI extends MainFrame {
  ...
  private var panelConversation = new BoxPanel(Orientation.Vertical) // là où la conversation sera enregistrée
  private var panelRequete = new TextField {                         // la zone de texte où l'utilisateur rentre sa demande
    maximumSize = new Dimension(Short.MaxValue, preferredSize.height)
  }
  
  private var scroll = new ScrollPane(panelConversation) // permet d'utiliser une barre de défilement
  private var barreScroll = scroll.verticalScrollBar     // la barre de défilement qui ne s'affichera que quand on en aura besoin
  
  private var iconeRequete = new ImageIcon("icones/interrogation.jpg") // avatars utilisateur = interrogation, machine = exclamation
  private var iconeReponse = new ImageIcon("icones/exclamation.jpg")
  ...
  private def gestionUI() {
    if (panelRequete.text != "") {
      ...
      panelConversation.contents += new BoxPanel(Orientation.Horizontal) { // panneau de réponse machine, collé à gauche
        ...
        contents += new Label {
          icon = iconeReponse 
          border = Swing.EmptyBorder(1, 1, 1, 5)
        }
